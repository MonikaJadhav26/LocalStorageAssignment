...
class CoreDataManager {
    ...
    func insertProjects(project : ProjectInfo,completion: @escaping (Result<Bool, Error>) -> Void)  {
        ...
        do {
            let result = try managedContext.fetch(fetchRequest)
            if (result.count > 0) {
                let updatedProject = (result[0] as! NSManagedObject) as! Project
                updatedProject.setValue(project.name, forKey: "name")
    ...
    func deleteEmployee(employeeId : String) {
        ...
        let fetchRequest = NSFetchRequest<NSFetchRequestResult>()
        let predicate = NSPredicate(format: "(id = %@)", employeeId)
        fetchRequest.entity = entity
        fetchRequest.predicate = predicate
        do {
            let result = try managedContext.fetch(fetchRequest)
    ...
    func deleteProject(projectName : String) {
        ...
        let entity = NSEntityDescription.entity(forEntityName: "Project",
                                                in: managedContext)!
        ...
        do {
            let result = try managedContext.fetch(fetchRequest)
            if (result.count > 0) {
                let project = (result[0] as! NSManagedObject) as! Project
